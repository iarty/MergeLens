...
describe('CommandPalette', () => {
  it('defines readable text colors independently from GitHub page styles', () => {
    const stylesheet = readFileSync(
      path.resolve(
        process.cwd(),
        'features/command-palette/CommandPalette.css',
      ),
      'utf8',
    )
    ...
    expect(stylesheet).toMatch(
      /\.command-palette__header h2\s*\{[^}]*color:\s*#1f2328/s,
    )
    expect(stylesheet).toMatch(
      /\.command-palette__input\s*\{[^}]*color:\s*#1f2328/s,
    )
    expect(stylesheet).toMatch(
      /\.command-palette__command\s*\{[^}]*color:\s*#1f2328/s,
    )
  })
  ...
  it('renders an accessible dialog and available commands', async () => {
    ...
    expect(
      screen.getByRole('option', { name: 'Open current pull request' }),
  ...
  it('filters commands without exposing unavailable actions', () => {
    ...
    expect(
      screen.getByRole('option', { name: 'Open MergeLens settings' }),
    ).toBeVisible()
    expect(
      screen.queryByRole('option', { name: 'Open repository' }),
  ...
  it('navigates with arrow keys and executes the selected command', async () => {
    ...
    expect(
      screen.getByRole('option', { name: 'Open repository' }),
    ).toHaveAttribute('aria-selected', 'true')
