...
describe('command palette context', () => {
  it('classifies a pull request page with its repository', () => {
    const context = createCommandContext('https://github.com/facebook/react/pull/42/files');
  ...
  it('classifies a repository page without inventing a pull request', () => {
    const context = createCommandContext('https://github.com/facebook/react/issues');

    expect(context?.repositoryContext).toEqual({ owner: 'facebook', repository: 'react' });
  ...
  it('rejects malformed and non-GitHub contexts', () => {
    ...
    expect(createCommandContext('https://example.com/facebook/react')).toBeNull();
