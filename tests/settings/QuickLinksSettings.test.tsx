...
describe('QuickLinksSettings', () => {
  ...
  it('adds a validated project link', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Label'), { target: { value: 'Preview' } });
    fireEvent.change(screen.getByLabelText('HTTPS URL'), { target: { value: 'https://preview.example.com' } });
    fireEvent.click(screen.getByRole('button', { name: 'Add link' }));
    await waitFor(() => expect(saveConfiguredQuickLink).toHaveBeenCalledWith(expect.objectContaining({
      label: 'Preview', url: 'https://preview.example.com/',
    })));
  });
  ...
  it('supports edit and remove actions', async () => {
    ...
    expect(editButton.parentElement).toHaveClass('quick-links-settings__actions');

    fireEvent.click(editButton);
    expect(screen.getByLabelText('Label')).toHaveValue('Docs');
    fireEvent.click(screen.getByRole('button', { name: 'Save link' }));
    await waitFor(() => expect(saveConfiguredQuickLink).toHaveBeenCalledWith(link));
    fireEvent.click(removeButton);
    await waitFor(() => expect(removeConfiguredQuickLink).toHaveBeenCalledWith('docs'));
  });
});
