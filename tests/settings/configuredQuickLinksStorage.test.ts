...
describe('configured quick links storage', () => {
  ...
  it('saves, sorts, updates, and removes links', async () => {
    ...
    expect((await readConfiguredQuickLinks()).map(({ id }) => id)).toEqual([
      'a',
      'z',
    ])
    await saveConfiguredQuickLink({
    ...
    expect(await removeConfiguredQuickLink('z')).toEqual([
      { id: 'a', label: 'Beta', url: 'https://b.example.com/' },
    ])
  })
