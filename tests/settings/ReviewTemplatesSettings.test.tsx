...
describe('ReviewTemplatesSettings', () => {
  ...
  it('renders accessible loading and empty states', async () => {
    let resolveList:
      | ((value: Awaited<ReturnType<ReviewTemplatesTransport['listTemplates']>>) => void)
      | undefined;
    ...
    expect(screen.getByRole('region', { name: 'Review templates' })).toHaveAttribute(
      'aria-busy',
      'true',
    );
  ...
  it('validates required fields and exposes configured limits', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Add template' }));
    expect(screen.getByRole('alert')).toHaveTextContent('Enter a template title.');
  ...
  it('blocks creation at the count limit while keeping edits available', async () => {
    ...
    expect(await screen.findByText(`100 / ${MAX_REVIEW_TEMPLATE_COUNT}`)).toBeVisible();
