...
import {
  PortableLocalDataSettings,
  type PortableLocalDataSettingsServices,
} from '@/features/settings/PortableLocalDataSettings'
import type { PortableLocalDataImportPreview } from '@/modules/portable-local-data'

const preview: PortableLocalDataImportPreview = {
  data: {
    format: 'mergelens-portable-local-data',
    schemaVersion: 1,
    ...
    data: {
      ...
      reviewNotificationPreferences: { enabled: false, intervalMinutes: 15 },
    },
  },
  additions: {
    notes: 1,
...
describe('PortableLocalDataSettings', () => {
  it('exposes accessible export and import controls', () => {
    ...
    expect(
      screen.getByRole('region', { name: 'Portable local data' }),
    ).toBeVisible()
  ...
  it('previews a file, requires conflict choice and confirmation, then applies import', async () => {
    ...
    fireEvent.change(fileInput, {
      target: {
        files: [
          new File(['{}'], 'portable.json', { type: 'application/json' }),
        ],
      },
    })
    ...
    const confirmation = screen.getByRole('checkbox', {
      name: /confirm this data/i,
    })
    fireEvent.click(confirmation)
    fireEvent.click(screen.getByRole('button', { name: 'Import data' }))
    await waitFor(() =>
      expect(services.applyImport).toHaveBeenCalledWith(
