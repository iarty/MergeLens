...
describe('ReviewNotificationsSettings', () => {
  ...
  it('requests optional permission from the enable gesture and persists consent', async () => {
    ...
    await waitFor(() =>
      expect(services.requestPermission).toHaveBeenCalledOnce(),
    )
    ...
    expect(
      await screen.findByText(
        'Background polling is scheduled every 15 minutes.',
      ),
    ).toBeVisible()
  })
  ...
  it('persists interval changes and disables without requesting permission', async () => {
    const services = createServices()
    vi.mocked(services.getPreferences).mockResolvedValue({
      enabled: true,
      intervalMinutes: 15,
    })
    ...
    await waitFor(() =>
      expect(services.savePreferences).toHaveBeenLastCalledWith({
        enabled: false,
        intervalMinutes: 30,
      }),
    )
