...
const createPayload = () => ({
  ...
  data: {
    ...
    repositoryWorkspacePreferences: [
      { repositoryKey: 'ACME/API', featureFlags: { savedFilters: true } },
    ],
...
describe('portable local data domain', () => {
  it('normalizes all durable categories and preserves only notification preferences', () => {
    ...
    expect(parsed.data.repositoryWorkspacePreferences[0]!.repositoryKey).toBe(
      'acme/api',
    )
    expect(parsed.data.reviewNotificationPreferences).toEqual({
      enabled: true,
      intervalMinutes: 30,
    })
  ...
  it('rejects future versions and unknown fields before persistence', () => {
    expect(() =>
      parsePortableLocalData({ ...createPayload(), schemaVersion: 99 }),
    ).toThrowError(expect.objectContaining({ code: 'unsupported-version' }))
    expect(() =>
      parsePortableLocalData({ ...createPayload(), unexpected: true }),
  ...
  it('rejects duplicates and invalid records', () => {
    ...
    expect(() =>
      parsePortableLocalData({
        ...
        data: {
          ...payload.data,
          reviewTemplates: [{ ...payload.data.reviewTemplates[0], body: '' }],
        },
      }),
    ).toThrowError(PortableLocalDataValidationError)
  })
  ...
  it('creates deterministic conflict keys for every category', () => {
    ...
    expect(getPortableDataConflictKey('notes', data.notes[0]!)).toEqual({
      category: 'notes',
      key: 'acme/api#2',
    })
    expect(
      getPortableDataConflictKey('reviewTemplates', data.reviewTemplates[0]!),
    ).toEqual({ category: 'reviewTemplates', key: 'template-b' })
    expect(
      getPortableDataConflictKey('savedFilters', data.savedFilters[0]!),
    ).toEqual({ category: 'savedFilters', key: 'filter-b' })
    expect(
      getPortableDataConflictKey(
        'repositoryWorkspacePreferences',
        data.repositoryWorkspacePreferences[0]!,
      ),
    ).toEqual({ category: 'repositoryWorkspacePreferences', key: 'acme/api' })
    expect(
      getPortableDataConflictKey(
        'globalWorkspacePreferences',
        data.globalWorkspacePreferences,
      ),
    ).toEqual({ category: 'globalWorkspacePreferences', key: 'singleton' })
    expect(
      getPortableDataConflictKey(
        'reviewNotificationPreferences',
        data.reviewNotificationPreferences,
      ),
    ).toEqual({ category: 'reviewNotificationPreferences', key: 'singleton' })
  })
})
