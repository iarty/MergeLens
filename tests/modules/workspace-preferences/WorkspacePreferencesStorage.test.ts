...
describe('WxtWorkspacePreferencesRepository', () => {
  ...
  it('subscribes to each preference category and cleans up watchers', () => {
    ...
    storageMock.watchers.get(GLOBAL_PREFERENCES_KEY)?.forEach((notify) => notify());
    storageMock.watchers.get(REPOSITORY_PREFERENCES_KEY)?.forEach((notify) => notify());
    ...
    unsubscribe();
    expect(
      [...storageMock.watchers.values()].every((listeners) => listeners.size === 0),
    ).toBe(true);
  });
