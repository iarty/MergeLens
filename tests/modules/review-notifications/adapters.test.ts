...
describe('review notification adapters', () => {
  ...
  it('deduplicates and bounds routes while disabling clears them', async () => {
    ...
    const routes = Array.from(
      { length: MAX_NOTIFICATION_ROUTES + 5 },
      (_, index) => ({
    ...
    await expect(repository.listRoutes()).resolves.toHaveLength(
      MAX_NOTIFICATION_ROUTES,
    )
  ...
  it('validates click targets as HTTPS GitHub pull requests', () => {
    expect(
      parseGitHubPullRequestUrl('https://github.com/acme/repo/pull/12'),
    ).toBe('https://github.com/acme/repo/pull/12')
    expect(() =>
      parseGitHubPullRequestUrl('http://github.com/acme/repo/pull/12'),
    ).toThrow()
    expect(() =>
      parseGitHubPullRequestUrl('https://example.com/acme/repo/pull/12'),
    ).toThrow()
    expect(() =>
      parseGitHubPullRequestUrl('https://github.com/acme/repo/issues/12'),
    ).toThrow()
  })
