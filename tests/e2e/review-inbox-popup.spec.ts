...
const installSuccessfulSearchRoutes = async (context: BrowserContext) => {
  await context.route('https://api.github.com/**', async (route) => {
    ...
    await fulfillJson(
      route,
      { message: 'Unexpected inbox fixture request' },
      404,
    )
  })
}
...
test('loads the review inbox and opens extension settings', async () => {
  ...
  try {
    ...
    await expect(optionsPage.getByLabel('Import JSON')).toBeVisible()
  } finally {
...
test('keeps partial inbox data and retries a failed section', async () => {
  ...
  try {
    ...
    await expect(
      popup.getByRole('link', {
        name: 'Clarify React server component warnings',
      }),
    ).toBeVisible()
...
test('applies a saved filter to inbox items and section counts', async () => {
  ...
  try {
    ...
    await expect(
      popup.getByRole('button', { name: /Assigned 1/ }),
    ).toBeVisible()
    await expect(
      popup.getByRole('link', {
        name: 'Improve concurrent rendering behavior',
...
test('persists a saved filter after reopening the settings surface', async () => {
  ...
  try {
    ...
    const optionsPage = await optionsPagePromise
    await expect(
      optionsPage.getByRole('heading', { name: 'Workspace preferences' }),
    ).toBeVisible()
    ...
    await reopened.goto(
      `chrome-extension://${getExtensionId(serviceWorker)}/options.html`,
      {
    ...
    await expect(
      reopened.getByRole('list', { name: 'Saved inbox filters' }),
...
test('applies a global feature-flag override without hiding other controls', async () => {
  ...
  try {
    ...
    await expect(
      popup.getByRole('region', { name: 'Command palette' }),
    ).toBeVisible()
    await expect(
      popup.getByRole('button', { name: 'Open command palette' }),
    ).toBeVisible()
  } finally {
    await closeExtension(context)
  }
