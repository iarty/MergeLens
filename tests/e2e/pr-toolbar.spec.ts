...
const deploymentsResponse = [
  {
    ...
    sha: 'fixture-sha-42',
    statuses_url:
      'https://api.github.com/repos/facebook/react/deployments/7/statuses',
  },
]
...
test('mounts exactly one toolbar across GitHub SPA navigation', async () => {
  ...
  try {
    ...
    await expect(
      page.getByRole('link', { name: 'React pull request 42' }),
    ).toBeVisible()
    await expect(page.getByText('Unit tests')).toBeVisible()
    await expect(
      page.getByRole('link', { name: 'Preview: success' }),
    ).toHaveAttribute('href', 'https://preview.example.com/react/42')
    ...
    await expect(
      page.getByRole('link', { name: 'React pull request 43' }),
    ).toBeVisible()
    await expect(toolbarHost).toHaveCount(1)
