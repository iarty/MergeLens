  ...
  try {
    ...
    const stderr =
      error instanceof Error && 'stderr' in error
        ...
        : String(error)
    throw new Error(
      `Extension build failed before Playwright startup: ${stderr}`,
    )
  }
}
...
export const launchExtension = async (): Promise<BrowserContext> => {
  const userDataDir = await mkdtemp(
    path.join(tmpdir(), 'mergelens-playwright-'),
  )
...
export const configureLocalCredential = async (
  ...
  await Promise.race([
    ...
    new Promise<never>((_, reject) => {
      setTimeout(() => {
        reject(
          new Error('Timed out while configuring extension-local test storage'),
        )
      }, 5_000)
    }),
  ])
  ...
  await serviceWorker.evaluate(async (nextRecords) => {
    await browser.storage.sync.set({
      ...
      ...(nextRecords.repositoryPreferences === undefined
        ? {}
        : {
            'workspacePreferences.repositories':
              nextRecords.repositoryPreferences,
          }),
    })
  }, records)
}
...
export const getExtensionId = (serviceWorker: Worker): string => {
  ...
  if (!extensionId) {
    throw new Error(
      'Unable to resolve MergeLens extension ID from service worker',
    )
  }
