...
describe('review notification background wiring', () => {
  ...
  const onAdded = createEvent<(permissions: { permissions?: string[] }) => void>();
  const onRemoved = createEvent<(permissions: { permissions?: string[] }) => void>();
  ...
  it('registers listeners synchronously, routes owned events, and cleans up', async () => {
    const repository = {
      getPreferences: vi.fn(async () => ({ enabled: true, intervalMinutes: 15 })),
      savePreferences: vi.fn(),
      getSnapshot: vi.fn(async () => ({ initialized: false, activeCandidateIds: [], updatedAt: null })),
      saveSnapshot: vi.fn(async () => undefined),
      listRoutes: vi.fn(async () => []),
      saveRoutes: vi.fn(async () => undefined),
      subscribePreferences: vi.fn(() => vi.fn()),
    };
    const source = { listReviewRequests: vi.fn(async () => ({ status: 'success', candidates: [] })) };
    const scheduler = {
    ...
    onAlarm.listeners.forEach((listener) => listener({ name: REVIEW_NOTIFICATION_ALARM_NAME }));
    await vi.waitFor(() => expect(source.listReviewRequests).toHaveBeenCalledOnce());
    onRemoved.listeners.forEach((listener) => listener({ permissions: ['notifications'] }));
    await vi.waitFor(() => expect(repository.saveRoutes).toHaveBeenCalledWith([]));
  ...
  it('starts without the optional notifications API and attaches after consent', () => {
    ...
    const repository = {
      getPreferences: vi.fn(async () => ({ enabled: false, intervalMinutes: 15 })),
      savePreferences: vi.fn(),
      getSnapshot: vi.fn(),
      saveSnapshot: vi.fn(),
      listRoutes: vi.fn(async () => []),
    ...
    onAdded.listeners.forEach((listener) => listener({ permissions: ['notifications'] }));
