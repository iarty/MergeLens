...
const createClient = (overrides: Record<string, unknown> = {}) => ({
  rest: {
    pulls: {
      get: vi.fn().mockResolvedValue({ data: { head: { sha: 'head-sha' } } }),
    },
    repos: {
      listDeployments: vi
        .fn()
        .mockResolvedValue({ data: [deployment], headers: {} }),
...
describe('OctokitQuickLinksReader', () => {
  ...
  it('loads bounded deployments, latest statuses, and configured links', async () => {
    ...
    expect(result).toMatchObject({
      status: 'success',
      data: { deployments: [{ state: 'success' }] },
    })
  ...
  it.each([
    ...
    const client = createClient({
      listDeployments: vi.fn().mockRejectedValue(error),
    })
    const reader = new OctokitQuickLinksReader(
      vi.fn().mockResolvedValue('token'),
      () => client as never,
