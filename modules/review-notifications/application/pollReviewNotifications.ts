...
export const createPollReviewNotifications = (
  ...
  const execute = async (
    ...
    if (!(await dependencies.permission.isGranted())) {
      logger.warn('Skipped review notification poll without permission', { trigger });
    ...
    const routesById = new Map(
      [...newRoutes, ...currentRoutes].map((route) => [route.notificationId, route]),
    );
