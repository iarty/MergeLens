...
export const registerReviewNotificationBackground = (
  ...
  const repository = overrides.repository ?? new WxtReviewNotificationStateRepository();
  const source = overrides.source ?? new OctokitReviewNotificationSource();
  const scheduler = overrides.scheduler ?? new BrowserReviewNotificationScheduler();
  const presenter = overrides.presenter ?? new BrowserReviewNotificationPresenter();
  const permission = overrides.permission ?? new BrowserNotificationPermissionGateway();
  const tabOpener = overrides.tabOpener ?? new BrowserReviewNotificationTabOpener();
  ...
  const onNotificationClosed = (notificationId: string) => {
    ...
    contain('close-route', repository.listRoutes().then((routes) =>
