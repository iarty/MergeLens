...
const snapshotSchema = z.object({
  ...
  activeCandidateIds: z.array(z.string().trim().min(1).max(128))
...
): ReviewNotificationSnapshot => snapshotSchema.parse({
...
export class WxtReviewNotificationStateRepository
implements ReviewNotificationStateRepository {
  ...
  private async getState() {
    const result = storedStateSchema.safeParse(await getItems().state.getValue());
    if (!result.success) {
  ...
  async saveSnapshot(snapshot: ReviewNotificationSnapshot): Promise<void> {
    ...
    const value = { snapshot: normalizeSnapshot(snapshot), routes: normalizeRoutes(current.routes) };
  ...
  async listRoutes(): Promise<ReviewNotificationRoute[]> {
    ...
    logger.debug('Read review notification routes', { routeCount: routes.length });
    return routes;
  }

  async saveRoutes(routes: ReviewNotificationRoute[]): Promise<void> {
    const current = await this.getState();
    const value = { snapshot: normalizeSnapshot(current.snapshot), routes: normalizeRoutes(routes) };
    const nextRecord = record(value);
    assertFitsQuota(nextRecord, 'state');
    await getItems().state.setValue(nextRecord);
    logger.info('Review notification routes saved', { routeCount: value.routes.length });
  }
