export const REVIEW_NOTIFICATION_ALARM_NAME = 'mergelens.review-notifications.poll';
...
export const createReviewNotificationDiff = (
  ...
  const newCandidates = currentCandidates.filter(({ id }) => !previousIds.has(id));
  const individualCandidates = newCandidates.slice(0, MAX_REVIEW_NOTIFICATIONS_PER_POLL);
