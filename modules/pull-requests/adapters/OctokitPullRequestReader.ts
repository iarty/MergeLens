...
const getRetryAfterSeconds = (
  ...
  if (retryAfter) {
    ...
    return Number.isFinite(parsedRetryAfter) ? Math.max(0, parsedRetryAfter) : undefined;
  }
...
const mapRequestError = (error: unknown): PullRequestReadResult => {
  if (error instanceof z.ZodError) {
    ...
    return {
      status: 'error',
      error: { code: 'invalid-response', message: 'GitHub returned invalid data' },
    };
  }

  if (isGitHubRequestError(error)) {
    if (error.status === 401) {
      logger.warn('GitHub rejected the configured token', { status: error.status });
      return {
        status: 'error',
        error: { code: 'unauthorized', message: 'GitHub token is not authorized' },
      };
    }
