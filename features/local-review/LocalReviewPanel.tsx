...
const stopEditorShortcut = (event: KeyboardEvent<HTMLTextAreaElement>): void => {
...
export const LocalReviewPanel = ({ controller }: LocalReviewPanelProps) => {
  ...
  useEffect(() => {
    setSelectedTemplateId((current) =>
      state.templates.some((template) => template.id === current) ? current : '',
    );
  }, [state.templates]);
    ...
    return (
      <section className="local-review" aria-label="Local review workspace">
        <div className="local-review__error" role="alert">
          ...
          <button className="local-review__button" type="button" onClick={controller.retry}>
    ...
    <section className="local-review" aria-label="Local review workspace">
      ...
      <div className="local-review__editor-group">
        <div className="local-review__label-row">
          ...
          <span>{state.draft.length.toLocaleString()} / {MAX_PULL_REQUEST_NOTE_LENGTH.toLocaleString()}</span>
        </div>
        <textarea
          ref={editorRef}
          ...
          aria-describedby={state.validationMessage ? 'mergelens-local-review-validation' : undefined}
      ...
      {state.validationMessage ? (
        <p id="mergelens-local-review-validation" className="local-review__validation" role="alert">
          {state.validationMessage}
        </p>
      ) : null}

      {state.saveStatus === 'error' && state.saveError && !state.validationMessage ? (
        <div className="local-review__inline-error" role="alert">
          <span>{errorTitle(state.saveError)}. {state.saveError.message}</span>
          <button className="local-review__text-button" type="button" onClick={controller.retry}>
            Retry save
          </button>
        </div>
      ) : null}
      ...
      <div className="local-review__tools">
        <div className="local-review__template-tool">
          <label htmlFor="mergelens-local-review-template">Review template</label>
          <div className="local-review__template-controls">
            ...
            >
              <option value="">
                {state.templates.length === 0 ? 'No templates available' : 'Choose a template'}
              </option>
      ...
      <div className="local-review__feedback" aria-live="polite">
        {state.clipboardStatus === 'success' ? 'Note copied to clipboard' : null}
