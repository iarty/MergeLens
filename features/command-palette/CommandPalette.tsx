...
export const CommandPalette = ({
  ...
  const titleId = useId();
  const listId = useId();
  const commands = useMemo(() => registry.search(query, context), [context, query]);
    ...
    <div className="command-palette__backdrop" role="presentation" onMouseDown={onClose}>
      ...
      >
        ...
        <input
          ...
          aria-activedescendant={selectedCommand ? `${listId}-${selectedCommand.id}` : undefined}
          value={query}
        ...
        />
        <div className="command-palette__results" id={listId} role="listbox" aria-label="Available commands">
          {commands.length === 0 ? (
            <p className="command-palette__empty" role="status">No commands found</p>
          ) : (
