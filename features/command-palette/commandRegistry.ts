...
export const createCommandRegistry = (commands: readonly Command[]): CommandRegistry => {
  const commandsById = new Map<CommandId, Command>();

  for (const command of commands) {
    if (commandsById.has(command.id)) {
      logger.warn('Rejected duplicate command registration', { commandId: command.id });
  ...
  const getAvailable = (context: CommandContext): readonly Command[] => {
    const availableCommands = commands.filter((command) => command.isAvailable(context));
    logger.debug('Resolved available commands', { count: availableCommands.length });
    return availableCommands;
  };

  const search = (query: string, context: CommandContext): readonly Command[] => {
